...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (int i = 0; i < pkgAppsList.size(); i++) {
            //XposedBridge.log("LALA: " + pkgAppsList.get(i).packageName);
        ...
        try {
            @SuppressLint("SdCardPath") File current_app = new File("/data/data/reaper.UIHarvester/current_app");
            if (!current_app.exists()) {
                current_app.createNewFile();
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
